...
class FavGroupTableViewCell: UITableViewCell {/******** here ********/
    ...
    @IBAction func favButtonPressed(_ sender: Any) {
        if let record  = record {
            showJoinAnimation()
            storeGroup(cell: self, record: record)/******** here ********/
        }
    }
    ...
    func storeGroup(cell: FavGroupTableViewCell, record: Record) {/******** here ********/
        //get the records
        ...
        if records.contains(where: {$0.id == record.id}) {
            ...
            UserDefaults.standard.set(encodedData, forKey: "groupRecords")/******** here ********/
